...
fn main() {
    let data: Vec<u8> = fs::read("input/libicudata.73.dylib").expect("Error reading file");
    ...
    // writer
    let mut writer = hound::WavWriter::create("output/libicudata.73.wav", spec).expect("Could not create writer");
